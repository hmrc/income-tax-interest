...
class CreateOrAmendAnnualIncomeSourcePeriodConnector @Inject()(http: HttpClient, val appConfig: AppConfig)(implicit ec: ExecutionContext) extends IFConnector {
  ...
  def createOrAmendAnnualIncomeSourcePeriod(
    ...
    def IFCall(implicit hc: HeaderCarrier): Future[CreateOrAmendAnnualIncomeSourcePeriodResponse] = {
      http.POST[InterestDetailsModel, CreateOrAmendAnnualIncomeSourcePeriodResponse](createOrAmendAnnualIncomeSourcePeriodUrl, interestModel)
    }
